...
<body class="bg-slate-900 text-slate-100 flex items-center justify-center min-h-screen p-4">
    <div class="w-full max-w-md bg-slate-800 border border-slate-700 rounded-xl p-8 shadow-2xl">
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-slate-400 mt-1">Acesso restrito ao gerenciador de repositórios</p>
        </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gerenciador de Repositórios - Admin</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
    ...
    <header class="bg-slate-900 border-b border-slate-800 sticky top-0 z-30 shadow-md">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="font-bold text-lg text-white">Repositórios</h1>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-8">
        ...
        <section class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
            <div class="mb-5">
                ...
                <p class="text-sm text-slate-400 mt-0.5">Cadastre URLs Git (HTTPS ou SSH). O nome é preenchido automaticamente se deixado em branco.</p>
            </div>

            <form action="/admin/repos" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div class="md:col-span-5">
                    <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1" for="link">Link do Repositório *</label>
                    <input type="text" id="link" name="link" required placeholder="https://github.com/usuario/meu-repo.git"
                ...
                <div class="md:col-span-3">
                    ...
                    <input type="text" id="name" name="name" placeholder="Auto-extraído do link se vazio"
                        class="w-full px-3.5 py-2 bg-slate-950 border border-slate-700 rounded-lg text-white text-sm placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none" />
                </div>
                <div class="md:col-span-3">
        ...
        <section class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
            <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-white text-lg">Repositórios Cadastrados</h3>
                    <p class="text-xs text-slate-400">Total: {{len .Repositories}} repositório(s)</p>
                </div>
                <div class="text-xs text-slate-500">
                    API: <code class="bg-slate-950 px-2 py-1 rounded text-indigo-400">/admin/api/repos</code>
                </div>
            </div>
                ...
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-950/60 text-xs uppercase font-semibold text-slate-400 border-b border-slate-800">
                        <tr>
                            ...
                            <th class="px-6 py-3.5">Chave de Acesso</th>
                            <th class="px-6 py-3.5">Cadastrado em</th>
                        ...
                        <tr class="hover:bg-slate-800/50 transition">
                            <td class="px-6 py-4 font-semibold text-white">
                                <span class="inline-flex items-center gap-1.5">
                                    <svg class="w-4 h-4 text-indigo-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/></svg>
                                    {{.Name}}
                                </span>
                            </td>
